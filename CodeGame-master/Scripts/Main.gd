...
func create_simulation():
	...
	simulationVariables = simulation.get_variables()
...
func process_assignment(line):
	...
	if varName.find(".") != null and varName.find(".") > 0:
		var varVariable = varName.split(".")[1]
		varName = varName.split(".")[0]
	var expression = line.split(" = ")[1]
	var expressionSections = expression.split(" ")
	
	for i in simulationVariables:
		if simulationVariables[i].name == varName:
			varName = simulationVariables[i]
			break
	
	if expressionSections.size() == 1:
		if simulationVariables.has(varName):
			if expression.is_valid_integer():
				simulationVariables[varName].varVariable = expression.to_int()
			elif expression.is_valid_float():
				simulationVariables[varName].varVariable = expression.to_float()
			elif expression[0] == "\"":
				simulationVariables[varName].varVariable = expression
				
		elif variables.has(varName):
			if expression.is_valid_integer():
